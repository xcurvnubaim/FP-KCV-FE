...
function ImageUploader(): JSX.Element {
  ...
  const handlePredict = async (): Promise<void> => {
    await predict(imagePreview); // Call the predict function with imagePreview as argument
  };
          ...
          >
            ...
            {imagePreview ? (
              <>
                ...
                <button onClick={handleClearImage} className="absolute top-2 right-2 text-gray-500 hover:text-gray-700">
                  X
                </button>
              </>
            ) : (
              <>
                <input ref={uploadInputRef} id="upload" type="file" className="hidden" accept="image/*" onChange={handleImageUpload} />
          ...
          <div className="flex items-center justify-center">
            <div className="w-full">
              <label className="w-full text-white bg-[#050708] hover:bg-[#050708]/90 focus:ring-4 focus:outline-none focus:ring-[#050708]/50 font-medium rounded-lg text-sm px-5 py-2.5 flex items-center justify-center mr-2 mb-2 cursor-pointer">
                <button className="text-center ml-2" onClick={handlePredict}>Upload</button>
              </label>
            </div>
          </div>
        </div>
