...
import { getDescFromArr, getDescription } from "@/utils/predict";
...
interface ImageUploaderProps {
  ...
  updateAnimalData: React.Dispatch<
    ...
    try {
      if (imagePreview) {
        ...
        const response = await axios.post(
          "https://api-animal-prediction.xcurv.cloud/predict/v2",
          formData,
          {
            timeout: 60000, // Increase timeout to 60 seconds (in milliseconds)
          }
        );
        setImagePreview("" + response.data.out);
        console.log("Success:", response.data);
        setImagePreview(
          "https://api-animal-prediction.xcurv.cloud/image/?image_name=" +
            response.data.out
        );
        setLoading(false);
        const description = await getDescFromArr(response.data.class_names);
            ...
            <div className="flex">
              ...
              <div className="w-full">
                <label className=" text-white bg-[#050708] hover:bg-[#050708]/90 focus:ring-4 focus:outline-none focus:ring-[#050708]/50 font-medium rounded-lg text-sm px-5 py-2.5 flex items-center justify-center mr-2 mb-2 cursor-pointer">
                  <button className="text-center" onClick={handlePredict}>
                    Upload
                  </button>
                </label>
              </div>
